...
class ScheduleMonthlyBase extends Report {

    public $date;
    public $squareHeight;
    public $squareWidth;
    public $longestMonth;
    public $tinySquareSize;

    public $sizeColumns = array(30,30,30,30,30,30,30);

    public function configCalendar() {
        $this->longestMonth = "";
    ...
    function printHoliday($date) {
        ...
        if (strlen($holiday)) {
            $wd = gmdate("w",jdtounix($date));
            if ($wd != 0 && $wd != 6) $this->Cell($this->squareWidth, $this->squareHeight, "", 0, 0, "", false);
